...
export default function MonthlyAttributionDetail({ year, month, hideBalance, onClose, onEdit }: Props) {
          ...
          <div className="bg-gradient-to-br from-blue-500 to-blue-600 rounded-xl p-5 text-white">
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <div className="text-lg font-bold">{formatHiddenAmount(lastNW, hideBalance)}</div>
              </div>
              <div className="text-2xl">→</div>
              <div>
                <div className="text-xs text-white/70">本月</div>
                <div className="text-lg font-bold">{formatHiddenAmount(currentNW, hideBalance)}</div>
              </div>
            </div>
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="font-medium">
                  {attribution.tags.map(tag => (
                    <span key={tag} className="mr-2">{getAttributionTagEmoji(tag)} {getAttributionTagLabel(tag)}</span>
                  ))}
                </span>
              </div>
              <div className="flex items-center gap-2">
                ...
                <span className={attribution.change >= 0 ? 'text-green-600' : 'text-red-500'}>
                  {attribution.change >= 0 ? '+' : ''}{formatHiddenAmount(attribution.change, hideBalance)}
                </span>
              </div>
              {attribution.note && (
                <div>
