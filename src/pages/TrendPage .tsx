...
export function TrendPage({ onPageChange }: TrendPageProps) {
  ...
  // 计算图表数据 - 修复X轴坐标错位问题：X坐标只基于有实际数据的月份计算
  const chartData = useMemo(() => {
    ...
    // 修复：X坐标只基于有hasData（实际数据）的点计算
    // 获取所有有实际数据的点
    const dataPoints = validHistory.filter(h => h.hasData !== false);
    const totalDataPoints = dataPoints.length;

    // 遍历所有 filteredHistory 点，X坐标基于有实际数据点的累计数量
    const points = filteredHistory.map((h, index) => {
      // 计算当前点之前有多少个有实际数据的有效点
      const dataPointsUpToIndex = filteredHistory.slice(0, index + 1)
        .filter(p => !p.isFiltered && p.hasData !== false).length - 1;

      // 基于有实际数据的总点数计算X坐标
      const x = totalDataPoints > 1 ? (dataPointsUpToIndex / (totalDataPoints - 1)) * 100 : 50;
      const y = 100 - ((h.netWorth - chartMin) / range) * 80 - 10;
      return { x, y, data: h, index };
    });

    // 只使用有实际数据的点绘制路径
    const validPoints = points.filter(p => !p.data.isFiltered && p.data.hasData !== false);
